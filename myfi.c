...
/* The event group allows multiple bits for each event, but we only
 * care about two events:
...
static void myfi_set_static_ip(esp_netif_t *netif)
{
    ...
    ESP_LOGD(TAG, "Success to set static ip: %s, netmask: %s, gw: %s",
             CONFIG_MYFI_IP, CONFIG_MYFI_NETMASK, CONFIG_MYFI_GATEWAY);
}
                          ...
                          int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_num < CONFIG_MYFI_RETRY_MAX) {
            esp_wifi_connect();
            ++s_retry_num;
...
void myfi_connect(void)
{
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ESP_LOGI(TAG, "connected to ap SSID: %s", CONFIG_MYFI_SSID);
    } else if (bits & WIFI_FAIL_BIT) {
        ESP_LOGI(TAG, "Failed to connect to SSID: %s", CONFIG_MYFI_SSID);
    } else {
